...
let tasks    = [];
let sessions = [];
let stats    = { total:0, todayDate:'', todayCount:0, streak:0, lastDate:'', weekData:{}, dailySeconds:{} };
let activeTaskIdx = -1;
...
// Removed wsSync - standalone desktop app
// Supabase Configuration
const { createClient } = require('@supabase/supabase-js');
const Dexie = require('dexie');
const { ipcRenderer } = require('electron');

// Listen for float window actions (toggle/reset)
ipcRenderer.on('float-action', (_e, action) => {
  if (action === 'toggle') toggleTimer();
  else if (action === 'reset') resetTimer();
});

// Listen for float window being closed natively
ipcRenderer.on('float-closed', () => {
...
async function save(){
  // Local fallback with Dexie
  await db.kv.bulkPut([
    { key: 'tasks', value: tasks },
    { key: 'sessions', value: sessions.slice(0, 200) },
...
async function loadLocalData(){
  try {
    const t = await db.kv.get('tasks');
    const s = await db.kv.get('sessions');
    const st = await db.kv.get('stats');
    const se = await db.kv.get('settings');
    
    tasks = t ? t.value : [];
    sessions = s ? s.value : [];
    stats = Object.assign({ total:0, todayDate:'', todayCount:0, streak:0, lastDate:'', weekData:{}, dailySeconds:{} }, st ? st.value : {});
    settings = Object.assign({ focus:25, short:5, long:15, interval:4, autoBreak:false, autoFocus:false }, se ? se.value : {});
  } catch(e) {
...
function recordElapsedSeconds(elapsed) {
  if (elapsed <= 0) return;
  const today = new Date().getFullYear() + '-'
    + String(new Date().getMonth()+1).padStart(2,'0') + '-'
    + String(new Date().getDate()).padStart(2,'0');
  stats.dailySeconds = stats.dailySeconds || {};
  stats.dailySeconds[today] = (stats.dailySeconds[today] || 0) + elapsed;
  save();
}
...
function pause(){
  ...
  let elapsed = sessionStartTimeLeft - timeLeft;
  if (elapsed > 0) {
    recordElapsedSeconds(elapsed);
    sessionStartTimeLeft = timeLeft;
  }
  document.getElementById('play-btn').innerHTML='&#9654;';
  updateModeTabs();
...
function updateDisplay(){
  ...
  if (endEl) {
    if (timeLeft > 0) {
      ...
      endEl.textContent = 'Ends at ' + d.toLocaleTimeString([], {hour: 'numeric', minute:'2-digit', hour12: true});
    } else {
      endEl.textContent = '';
    }
  }
...
/* ─ Analytics ─ */
function getDailyHours(ds) {
  stats.dailySeconds = stats.dailySeconds || {};
  if (stats.dailySeconds[ds] !== undefined) {
    return stats.dailySeconds[ds] / 3600;
  }
  const count = (stats.weekData && stats.weekData[ds]) || 0;
  return (count * settings.focus) / 60;
}
...
function renderAnalytics(){
  ...
  const todayHrs = getDailyHours(todayStr).toFixed(1);
  document.getElementById('a-today').textContent=todayHrs;
  ...
  stats.dailySeconds = stats.dailySeconds || {};
  stats.weekData = stats.weekData || {};
  const allDays = new Set([...Object.keys(stats.dailySeconds), ...Object.keys(stats.weekData)]);
  
  for (const ds of allDays) {
    if (ds.startsWith(currentMonthPrefix)) {
      monthlyHrs += getDailyHours(ds);
    }
  }
  
  document.getElementById('a-total').textContent=monthlyHrs.toFixed(1);
...
function renderChart(){
  ...
  const maxS=Math.max(1,...days.map(d=>getDailyHours(d)));
  days.forEach(ds=>{
    const hrs=getDailyHours(ds);
    const hp=hrs>0?Math.max(6,(hrs/maxS)*100):0;
    const isToday=ds===tStr;
    const dow=new Date(ds+'T00:00:00').getDay();
    const label=dn[(dow+6)%7];
    const g=document.createElement('div'); g.className='bar-col';
    const barClass='bar-fill'+(isToday?' is-today':hrs>0?' has-data':' is-empty');
    const displayVal = hrs > 0 ? parseFloat(hrs.toFixed(1)) + 'h' : '';
    g.innerHTML=`<span class="bar-count">${displayVal}</span>
...
function renderHeatmap(){
  ...
  stats.dailySeconds = stats.dailySeconds || {};
  stats.weekData = stats.weekData || {};
  const allDaysSet = new Set([...Object.keys(stats.dailySeconds), ...Object.keys(stats.weekData)]);
  const activeDates = Array.from(allDaysSet).filter(d => getDailyHours(d) > 0).sort();
  ...
  let yearlyHrs = 0;
  for (const d of activeDates) {
    if (d.startsWith(currentYearPrefix)) {
      yearlyHrs += getDailyHours(d);
    }
  }
  
  document.getElementById('hm-total').textContent       = yearlyHrs.toFixed(1);
  ...
  weeks.forEach((week) => {
    ...
    week.forEach(dayStr => {
      const hrs    = getDailyHours(dayStr);
      const isFuture = dayStr > todayStr;
      const isToday  = dayStr === todayStr;
      const level    = isFuture ? 'future' : getLevel(hrs);
      ...
      const tipText = isFuture
        ? dateLabel
        : hrs===0
          ? `0 hours worked — ${dateLabel}`
          : `${hrs.toFixed(1)} hours worked — ${dateLabel}`;
...
(async function(){
  ...
  if (supabaseClient) {
    ...
    // Offline capability sync mechanism — pull remote first, then push local changes
    window.addEventListener('online', async () => {
      showToast("Online: Syncing data...");
      // Pull remote data first so we don't blindly overwrite changes from other devices
      await syncDataFromDatabase();
      // Push any local changes that occurred while offline
...
function toggleFloatingTimer(enable) {
  const cb = document.getElementById('s-floating');
  ...
  if (enable) {
    ...
    updateFloatingTimer();
    showToast('Floating timer opened.');
  } else {
    ipcRenderer.send('close-float');
    showToast('Floating timer closed.');
  }
}

// Restore floating timer on startup
